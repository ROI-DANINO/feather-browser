# ADR-0010 — Local Control-Plane and Capability Model (CANDIDATE — NOT Accepted)

- **Date:** 2026-06-07
- **Status:** 🚧 **PROPOSED — NOT ACCEPTED.** This ADR records the security model the
  2026-06-07 council review (5/5 models) said must exist *before* Feather exposes its
  highest-privilege surfaces. It is the **capability/security model** that the council's
  "security model first, interop through that model" reversal points to. Acceptance is a
  joint call (Roi + agent); **Phase 5.0 implements it.**
...
- **Single-use or short TTL** — a grant authorizes one attach/unlock, or expires in seconds.
...
- **Human-approved at grant time** — minting a grant requires an explicit human action, not the mere
  presence of the bearer token.
- **Audited** — `grant.requested` / `granted` / `denied` / `used` / `expired` / `revoked` events
  (redacted) on the logging bus and a durable audit surface.
...
## Open questions (joint call at acceptance)

1. **Capability-token shape.** Single-use opaque nonce minted by Feather (lean) vs. short-TTL signed
   token vs. a `0600` capability file. Lean: an opaque single-use token Feather mints and surfaces to
   the **human** to approve, never to the agent.
2. **Human-approval surface in Phase 4a (no shell yet).** Console prompt + a local approval page
   (same pattern as the MFA local page) vs. Telegram confirm. Likely: reuse the MFA local-page
   pattern so there is one coordination surface.
3. **Ship the global `Origin`/`Host` hook immediately?** It is cheap and closes the worst hole; a case
   exists to land it as a small hardening PR ahead of the full Phase 5.0 build.
4. **Audit durability.** Logging-bus events only (ephemeral) vs. a dedicated append-only audit log
   under the STATE root. Dangerous-tier actions argue for the durable surface.
