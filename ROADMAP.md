...
| **5d.4 — Learned human-like input → STEALTH** | agent replays *your* captured dynamics; re-measure vs 5d.2 (spike-first) | future design — **mouse-motion synthesis via CDP `Input.dispatchMouseEvent`** (gap = trajectory SHAPE; `isTrusted` already passes). **GATED by 2 verifications** (Chrome-136 attach — spawn path OK; **`Runtime.enable` leak** vs rebrowser — if it leaks, outranks motion). Width: DataDome/HUMAN win, ~zero for Cloudflare. Research: `research/2026-06-23-bot-detection-landscape-research.md` |
