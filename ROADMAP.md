...
| **5d.4 — Learned human-like input → STEALTH** | agent replays *your* captured dynamics; re-measure vs 5d.2 (spike-first) | future design — **mouse-motion synthesis via CDP `Input.dispatchMouseEvent`** (gap = trajectory SHAPE; `isTrusted` already passes). **Gate (b) CLEARED 2026-06-23: 🟢 no `Runtime.enable` leak** (rebrowser) → un-blocked, no patch/spoof-exception needed. Width: DataDome/HUMAN win, ~zero for Cloudflare. NEW cheap static win queued: run real Google Chrome (rebrowser 🔴 `useragent` = bundled Chromium). Research: `research/2026-06-23-bot-detection-landscape-research.md` |
