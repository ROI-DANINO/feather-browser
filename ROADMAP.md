...
| **5d.4 — Learned human-like input → STEALTH** | agent replays *your* captured dynamics; re-measure vs 5d.2 (spike-first) | future design — **mouse-motion synthesis via CDP `Input.dispatchMouseEvent`** (gap = trajectory SHAPE; `isTrusted` already passes). **Gate (b) RAN 2026-06-23 — detectors DISAGREE:** rebrowser `runtimeEnableLeak` 🟢 clean BUT **Brotector `runtime.enabled` 🔴 score 1.00 DETECTED** (`nameLookupCount`) → **CDP-runtime hardening re-opened, possibly outranks 5d.4** (a CDP tell scoring 1.00 makes mouse-motion moot on that detector). `Input.untrusted` PASS. Width: DataDome/HUMAN win, ~zero Cloudflare. Static win queued: real Google Chrome (🔴 `useragent`). Detail: `docs/testing/5d2-baseline/baseline-report.md` Addendum C; research: `research/2026-06-23-bot-detection-landscape-research.md` |
