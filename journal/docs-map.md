...
| `ROADMAP.md` | Destination, phase/session execution map, milestones, exit criteria. Completed-phase detail -> `journal/ops/archive/`. |
| `journal/context/active.md` | **Current state + next action (the single owner).** Resume context for `/start`. |
| `journal/context/next.md` | Active short-term bridge between `/next` snapshots and the next `/stop`. Holds only pending bridge entries. |
...
| `journal/ops/sessions/` | Durable `/stop` handoff files (append-only history). |
| `journal/ops/archive/` | Rotated-out history (log, Phase-3 progress, completed roadmap phases). |
| `journal/archive/` | Archived `/next` bundles and other superseded temporary handoff material. Preserved history; not read by default during `/start`. |
