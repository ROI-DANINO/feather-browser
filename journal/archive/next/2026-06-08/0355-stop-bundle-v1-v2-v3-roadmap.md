...
---
## 2026-06-08 01:14 — Demo first-run login fix (unblocks 4a.9 recording)

### Session pointer
- Phase 4a. **Tangent that serves the real goal:** fixed the hero demo so a *cold* first run works
  end-to-end → Roi is now recording the demo clip (4a.9). The planned doctrine-reconciliation pass
  (2026-06-08 00:21 entry below) is **still pending** — its paste-prompt is unchanged and stands.

### Summary
- The hero demo "messed up on first try": logging into Google bounced the tab to the
  workspace.google.com marketing page and the login looked un-remembered, forcing a re-login.
- Root cause = the continuity poll loop **re-navigated to the Gmail URL every 10s** while the human
  was mid-login, interrupting Google's OAuth redirect chain. Fixed → first run now works first-try.
- Roi will **record the demo warm** (one cold warm-up run to dismiss first-run popups, then record
  take 2) for a cleaner clip. He's fine leaving the popups for the cold case.

### Completed
- `scripts/demo/continuity.ts`: poll loop now **probes the current page** for the compose button
  instead of re-navigating; removed the dead `sleep` helper. (`wait until visible` returns the
  instant the element appears, so detection stays near-instant.)
- `scripts/demo/hero-chatgpt-gmail.ts`: confirmed/kept `GMAIL_URL = mail.google.com` (a brief
  workspace.google.com experiment was reverted — wrong fix).
- Reframed the "double run" notes in `README.md` + `run-hero-demo.sh`: first run works, warm re-run
  is **optional polish** for recording (skips login + first-run popups), not a requirement.
- Deleted the stale cached profile at `~/.local/share/feather/profiles/primary/profile` at Roi's
  request before he re-recorded.
- Committed: `ec10e52 fix(demo): stop re-navigation interrupting Google login on first run` (3 files).
  **Not pushed** (per dev/master policy — push deferred to `/stop`).

### User decisions / quotes
- Decision: record the demo **warm** for a nicer clip. Quote: "i think ill run it warm, but fix the
  note on the dubble demo run cause it works first try".
- Decision: accept the cold-run popups (sign-in promo + mailto handler), don't chase suppressing them.
  Quote: "now it will work on the first run, so im okay with the popups".

### Agent decisions / assumptions / rationale
- Diagnosed from the code + Roi's screenshots (can't play video files). The re-navigation was the
  load-bearing bug; removing it (not changing URLs) was the real fix.
- On the two cold-run popups (Chromium sign-in promo, mailto handler): researched — **no clean CLI
  flag** for either; they're profile-pref/policy controlled and **first-run-only**. Recommended the
  warm-take-2 workflow over fragile pref-seeding; Roi agreed.
- Did NOT add `--test-type` / cosmetic flags to `modes.ts` — kept the anti-detection arg surface
  untouched (would've been demo-gated if pursued).

### Files read or touched
- Read: `scripts/demo/{run-hero-demo.sh,hero-chatgpt-gmail.ts,continuity.ts}`, `src/browser/modes.ts`,
  `tsconfig.json`, `docs/specs/2026-06-06-hero-demo-workflow-design.md`.
- Touched: `scripts/demo/continuity.ts`, `scripts/demo/run-hero-demo.sh`, `README.md`.
- Untracked, left alone: `demo.mp4` (Roi's recording in progress).

### Open threads / unresolved questions
- **Doctrine reconciliation pass (00:21 entry) is still the next real task** — unchanged paste-prompt.
- 4a.9 itself: once Roi has the clip, the GIF/video still needs to land in the README / LinkedIn post.
- Cold-run popups remain on a truly fresh profile (accepted, not fixed).

### Next action
- Roi finishes recording the demo clip. THEN: open a fresh chat and run the doctrine-reconciliation
  pass (paste-prompt in the 2026-06-08 00:21 entry below), or wire the recorded clip into the README
  if 4a.9 is the priority.

### Next session should read
- This entry + the 2026-06-08 00:21 entry (doctrine pass paste-prompt).
- `scripts/demo/continuity.ts` (if revisiting demo robustness).

### Risks / blockers
- Planning-spiral risk still flagged — this session was a small, healthy detour that actually moved
  the real deliverable (recordable demo). Keep the demo clip landing before re-entering planning.

---
## 2026-06-08 02:25 — Hero demo recording & README integration (4a.9 DONE)

### Session pointer
- Phase 4a; **Session 4a.9 - LinkedIn debut recording COMPLETE**. Next = integration-doctrine pass (4a.7/4a.8/5d/5e re-tagging), then 4a.8 Crawl4AI port.

### Summary
- Roi wanted to record the demo. Installed wf-recorder, recorded demo.mp4, edited in Kdenlive, used FFmpeg to trim+speedup (1.25x), added to README.
- Demo video is now in repo and linked in Hero Demo section of README.md.
- **4a.9 deliverable: done and shipped.**

### Completed
- Installed wf-recorder (Wayland screen recorder) via `dnf install -y wf-recorder`.
- Recorded live demo run with `npm run demo:hero` using wf-recorder.
- Trimmed + sped up video to 1.25x with FFmpeg (14.5s start, 35s duration → 28.37s final).
- Fixed VP9 keyframe issues by re-encoding with MPEG4 codec.
- Added demo-final.mp4 link to README.md Hero Demo section.
- Committed: `52b63fe demo(4a.9): Hero demo recording — ChatGPT to Gmail flow`.
- Pushed to origin/dev.

### User decisions / quotes
- Decision: record the demo (not defer). Quote: "i want to record the damn demo".
- Wanted video edited: start cut to 14.5s, length 35s, sped 1.25x.
- Chose wf-recorder over Kooha (available in Fedora repos, simpler).
- Added to README hero section (not buried elsewhere).

### Agent decisions / assumptions / rationale
- Recommended FFmpeg trimming + speedup over re-recording (reused existing video).
- Diagnosed and fixed keyframe jumping (VP9 copy → re-encode with MPEG4).
- Used MPEG4 codec (only stable MP4 encoder on this FFmpeg build).
- Kept demo video in repo root for simple README link (not in examples/ or docs/).

### Files read or touched
- Read: `README.md`, package.json, scripts/demo/run-hero-demo.sh.
- Touched: `README.md` (added demo-final.mp4 link), demo-final.mp4 (created).
- Left alone: WhatsApp Video 2026-06-08 at 02.15.45.mp4 (intermediate cut).

### Open threads / unresolved questions
- None. 4a.9 is closed.

### Next action
- Run the integration-doctrine reconciliation pass (from 2026-06-08 00:21 entry). Brainstorm-first on build-vs-buy rule, per-repo disposition table, re-tag 4a.7/4a.8/5d/5e.

### Next session should read
- The 2026-06-08 00:21 entry (doctrine pass paste-prompt).
- `research/2026-06-07-open-source-integration-research.md` (refresher on repo tracks).
- `docs/specs/adr-0006-agent-interface-neutrality.md` (check for doctrine overlap).
