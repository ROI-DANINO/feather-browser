...
- **Scratch IG identity CHANGED:** old `feather_test_roi` no longer existed; Roi created a **brand-new `roionly9` IG (`[REDACTED-PW]`)** (2026-06-11), same handle as the scratch Google. Logged in via the daily-driver window (Roi typed creds), then `daily:stop -- scratch` saved the IG session to disk cleanly + released the lock. (NB feed already shows popular seeded content — IG seeds new accounts.)
