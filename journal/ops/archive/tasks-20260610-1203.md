...
Latest (2026-06-10 ~09:50 STOP): **Native Capabilities Router PLACED at Session 5.0.1, after Gate A**
(Roi confirmed; research doc + pointers landed; "Connector Registry" naming; doctrine ADR question deferred
to the 5.0.1 joint call). Earlier same day: command layer upgraded (`/blog`+`/notebook`+blog gate+CLAUDE.md
fix, thru `3cb86dc`); Graphify GRADUATED to `dev`; NotebookLM Project Brain v2 SHIPPED; pass-2 observe
measurement DONE (9 PASS + M1 PARTIAL, zero ELEMENT_NOT_FOUND). Open follow-ups, in order:
...
- [x] **NotebookLM Project Brain v2 — SHIPPED (2026-06-10, `graphify-test` worktree; now on `dev`).**
...
- [x] **Graphify GRADUATED to `dev` (2026-06-10).** Keep/discard gate = KEEP; merge `e3de005` pushed.
      Path-agnostic post-commit hook enabled (`core.hooksPath .githooks`); graph live in main
      `graphify-out/` (rebuild with `graphify update .`, NOT `extract`); MCP repointed (local scope);
      worktree + branch deleted. Session
      `journal/ops/sessions/graphify-graduated-to-dev-20260610-0652.md`; blog `0018`.
- [x] **Native Capabilities Router — PLACED (2026-06-10 ~09:50, Roi confirmed).** Anchored at Session
      5.0.1 after Gate A; research doc `research/2026-06-10-native-capabilities-router.md`; inbox
      originals archived; "Connector Registry" naming; docs-import builder = v3/5e. Session
      `journal/ops/sessions/native-capabilities-placed-20260610-0950.md`.
- [x] **Command layer: /blog + /notebook + /stop blog gate (2026-06-10, pushed thru `3cb86dc`).**
      /stop never blogs without an explicit yes (declined → `blog/_pending.md` owed line); /blog =
      scan-first catch-up; /notebook = pack refresh + manual re-upload checklist. Graphify verdict:
      no skill needed (MCP self-serve). Bonus fix: root `CLAUDE.md` with `@AGENTS.md` import —
      AGENTS.md wasn't auto-loading before. Spec/plan `docs/{specs,plans}/2026-06-10-blog-notebook-commands*`.
...
- [ ] **5.0.1 — MCP & tool-surface reconciliation** — now also owns the **Native Capabilities
      Router / Connector Registry** decision (placed 2026-06-10, after Gate A; input
      `research/2026-06-10-native-capabilities-router.md`; say "Connector Registry" not
