...
4. [ ] **Warmed persistent Google session on disk ▶ NEXT (substantive)** — long-running primary authenticated context (Cookie Mine foundation); single-click Google Auth, agent blind, Feather injects under the hood. **Needs Roi (one-click Google login) + run the cookie-isolation spike first** (`research/2026-06-04-cookie-jar-isolation-and-phase5-sequencing.md` → "Queued action").
