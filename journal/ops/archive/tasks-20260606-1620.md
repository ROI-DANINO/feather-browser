...
Active track: **Phase 4a SHIPPED** (2026-06-05) → next = **merge `dev`→`master`** (milestone
graduation, Roi's call; dev verified stable this session), then **LinkedIn debut polish** in a
separate session, then **Phase 4b** (shell). Phase 4a executed `docs/plans/2026-06-05-core-first-reorientation.md`
end-to-end (7 commits `8d42aab`..`16f5ab7`, pushed); public README + demo live on default branch `dev`.

## Next — milestone graduation + debut (immediate)
- [ ] **Merge `dev`→`master`** — graduate this milestone (Roi's call, like PR #1). Gate: confirm dev
  stable (already green this stop: 184u + tsc 0 + demo green, tree clean, pushed). `master` is 33 behind.
- [ ] **LinkedIn debut polish** (separate session) — optional demo GIF/asciinema of `quickstart.sh`;
  any final README touch-ups; then actually post. (Public README + demo are already live on `dev`.)
