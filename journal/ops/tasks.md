...
**Next action (2026-06-23 21:42 STOP) = ISOLATE the Brotector CDP-runtime detection, then decide
CDP-hardening vs 5d.4 sequencing.** 5d.1 stealth base SHIPPED (TDD, pushed). 5d.2 measured live:
static axis PASS (incl. commercial Fingerprint Pro, `tampering_ml_score: 0` = verify-don't-spoof
validated); behavioral gap = mouse-motion; **but the CDP-leak axis is NOT closed — rebrowser said
clean, Brotector said DETECTED (1.00, `runtime.enabled`/`nameLookupCount`).** Next: (1) does it fire
with ZERO Feather evaluates (navigate+idle)? (2) does rebrowser-patches-style `Runtime.enable`
suppression defeat the `nameLookupCount` method? If a CDP tell scores 1.00 regardless of behavior it
likely **outranks 5d.4**. Report: `docs/testing/5d2-baseline/baseline-report.md` (gate v1→v4). All
commits on `dev`, pushed. _(Older next-actions below superseded.)_

**(superseded) Next action (2026-06-23 20:23) = BUILD 5d.1 (stealth base).** 5d reconciled to
**secure-only** + the full **stealth/Behavior-Mine arc** set into the roadmap (5d.1→5d.5; arc design
