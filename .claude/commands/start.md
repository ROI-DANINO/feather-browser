# /start — Resume Feather Browser Work

Run each step before doing anything else.

1. Read in parallel:
   - `README.md`
   - `ROADMAP.md`
   - `PROGRESS.md`
   - `schema.md`
   - `ops/tasks.md`
   - `ops/phase.md`
   - `context/active.md`
   - last 20 lines of `log.md`
2. Check `raw/_inbox/` for files other than `README.md`. If any exist, report the count and ask whether to process them now or continue.
3. Report:
   - Last known phase
   - Current in-progress or active task
   - Next concrete action
   - Relevant desk: `product`, `browser`, `automation`, or `general`
4. Ask before loading desk context from `work/<desk>/context.md`.
5. Ask: "Ready to continue, or do you want to start somewhere else?"

Do not begin work until the user confirms.

